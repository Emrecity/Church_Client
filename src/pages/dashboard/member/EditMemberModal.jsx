...
import { EditMemberSchema } from '../../../helpers/Schemas'
...
const [img,setImg] = useState(data?.image||null)
...
const{mutate} = useMutation({
  ...
  mutationFn: async (data) => {
      const res = await axios.patch(`api/v1/member/${data?.id}`,data,{
            headers:{
                'Content-Type':'multipart/form-data'
            }
        })
      if(res?.status == 200){
        toast.success('Member Updated Successfully',500)
  ...
  return (
    <Modal closeModal={closeModal} modal_id='edit_member_modal'>
      <div className="p-5 overflow-hidden bg-white border rounded-sm border-stroke shadow-default dark:border-strokedark dark:bg-boxdark">
        <h1 className="text-2xl text-center text-blue-500 poppins-bold">Edit Member</h1>
        <form onSubmit={handleSubmit(submitHandler)}>
          <div className="flex flex-col poppins-regular-italic gap-y-5">
           {
             data.image && !toggle?<><img src={data.image}/> <button onClick={()=>setToggle(!toggle)} className="p-1 text-white bg-blue-500 rounded-md">Change Picture</button></>:<>
             {!img?<div
             id="FileUpload"
           ...
           >
            ...
            <div className="flex flex-col gap-y-2">
              ...
              <input type="text" defaultValue={data?.firstname} placeholder="Enter first name" className="form-control" {...register('firstname')} />
              <small className="text-red-300">{errors.firstname?.message}</small>
            </div>
            <div className="flex flex-col gap-y-2">
                <label>Last Name</label>
                <input type="text" defaultValue={data?.lastname} placeholder="Enter last name" className="form-control" {...register('lastname')} />
                <small className="text-red-300">{errors.lastname?.message}</small>
            </div>
            <div className="flex flex-col gap-y-2">
                    <label>Other Name</label>
                    <input type="text" defaultValue={data?.othername} placeholder="Enter other name" className="form-control" {...register('othername')} />
            </div>
            <div className="flex flex-col gap-y-2">
                    <label>Telephone</label>
                    <input type="tel" defaultValue={data?.phone} placeholder="(+233 57 589 2390)" className="form-control" {...register('phone')}/>
            ...
            <div className="flex flex-col gap-y-2">
                    ...
                    {!dateToggle?<input type="text" value={formatdate} onChange={(e)=>setDateToggle(true)} className="form-control"/>:
                    <input type="date" placeholder={date} className="form-control" {...register('dateOfBirth')} />}
                    <small className="text-red-300">{errors.dateOfBirth?.message}</small>
            </div>
            <div className="flex flex-col gap-y-2">
                    <label>Age Range</label>
                    <select className="form-control" {...register('ageRange')}>
                    <option value={data?.ageRange}>{data?.ageRange}</option>
            ...
            <Button btnText='Update Member' cssClass='btn-primary my-1'/>
            <button type="button" className="w-full p-1 my-1 text-white bg-red-500" onClick={handleClose}>Close</button>
          </div>
        </form>
      </div>
    </Modal>
